...
INGREDIENT_DETAILS = {
  ...
  'Azelaic Acid': 'A clinical active that targets skin redness, fades blemishes, and controls acne bacteria without barrier harm.',
...
TRADITIONAL_INGREDIENTS = [
  { "id": 'saffron', "name": 'Saffron (Kesar)', "icon": '🌸', "color": '#D18A6A', "desc": 'Prestige golden stamens for pristine cell warmth' },
  { "id": 'turmeric', "name": 'Turmeric (Haldi)', "icon": '🌻', "color": '#E5A93B', "desc": 'Healing yellow root for gold ubtan purity' },
  { "id": 'sandalwood', "name": 'Sandalwood (Chandan)', "icon": '🪵', "color": '#8B9474', "desc": 'Creamy fragrant timber that pacifies and cools' },
  { "id": 'rosewater', "name": 'Rose Water (Gulab Jal)', "icon": '💧', "color": '#BD808A', "desc": 'Slow steam-distilled hydrating floral dew' },
  { "id": 'neem', "name": 'Neem Leaves', "icon": '🌿', "color": '#4E6E58', "desc": 'Sacred green leaves for deep clearing power' },
  { "id": 'honey', "name": 'Forest Honey (Madhu)', "icon": '🍯', "color": '#C97D54', "desc": 'Ancient natural glaze that locks moist dews' },
]

def get_remedy_recipe(selected_ingredients):
    if not selected_ingredients:
        return None
    ingredients_set = set(selected_ingredients)
    if 'saffron' in ingredients_set and 'turmeric' in ingredients_set and 'sandalwood' in ingredients_set:
        return {
    ...
    if 'neem' in ingredients_set and 'sandalwood' in ingredients_set:
        return {
    ...
    if 'rosewater' in ingredients_set and 'saffron' in ingredients_set:
        return {
    ...
    if 'honey' in ingredients_set and 'rosewater' in ingredients_set:
        return {
    ...
    if 'neem' in ingredients_set and 'honey' in ingredients_set:
        return {
    ...
    if 'saffron' in ingredients_set and 'sandalwood' in ingredients_set:
        return {
    ...
    if 'turmeric' in ingredients_set:
        return {
...
NEWS_ITEMS = [
  {
    "id": "news-1",
    "source": "Lancet Dermatology Research",
  ...
  {
    "id": "news-3",
    "source": "Harvard Skin Science Quarterly",
  ...
  {
    "id": "news-4",
    "source": "Aesthetic Medical Review",
